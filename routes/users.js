  ...
  try {
    ...
    if(foundUser) {
      res.json({
        ...
        message: 'Pincode retrieve successfully',
        success: true
      })
    } else {
      res.json({
        pincode: '',
        message: 'Pincode retrieve failed',
        success: false
      })
    }
  } catch(err) {
...
router.post('/pincode-edit', async(req ,res) => {
  try {
    const id = req.body.id
    const oldPincode = req.body.oldPincode
    const newPincode = req.body.newPincode
    const foundUser = await User.findById(id);
    if(foundUser) {
      if(oldPincode == foundUser.pincode) {
        foundUser.pincode = newPincode;
        const changePincode = await foundUser.save()
        res.json({
...
// @desc    Login user
// @access  Public
router.post('/login-mobile-call', async(req, res) => { 
  // Error Validation

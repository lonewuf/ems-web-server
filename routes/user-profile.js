...
router.post('/leaves-mobile-call', passport.authenticate('jwt', { session: false }),(req, res) => {
  console.log("aaaaa")
  var date_from = req.body.date_from
  var date_to = req.body.date_to
  var leave = req.body.leave;
  ...
  if(date_from == '') {
    ...
    if(date_to == '') {
      ...
      if(leave == '') {
        res.json({
          message: 'Type of date is required',
          success: false
        });
      } else {
        if(reason == '') {
          ...
          if(date_from >= date_to) {
            ...
            if(daysLeave <= 0) {
              ...
              const typeOfLeave = leave == 'sick_leave' ? 'sick_leave' : leave;
              var objLeaveError = {}
              var stateOfLeave;
              if(typeOfLeave == 'sick_leave') {
                if(date_to.getTime() < dateNow.getTime() && date_from.getTime() < dateNow.getTime())
                  stateOfLeave = true
                else {
                  objLeaveError.message = 'Sick Leave cannot be filed in future'
                  objLeaveError.success = false
                }
              } else {
                if(date_to.getTime() > dateNow.getTime() && date_from.getTime() > dateNow.getTime())
                  stateOfLeave = true
                else {
                  objLeaveError.message = 'Vacation Leave cannot be filed in past'
                  objLeaveError.success = false
                }
              }
...
router.get('/user-mobile-call', passport.authenticate('jwt', { session: false }), (req, res) => {
  User.findOne({email: req.user.email})
    .populate('reports') 
    .populate('leaves')
    .then(foundUser => {
